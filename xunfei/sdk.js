...
let storageSecrets = JSON.parse(localStorage.getItem("xunfei-secrets") || "{}")

//APPID，APISecret，APIKey在控制台-我的应用-语音合成（流式版）页面获取
const APPID = storageSecrets.id;
const API_SECRET = storageSecrets.secret;
const API_KEY = storageSecrets.key;

if (storageSecrets.id == null) {
  console.error("调用 API 要钱的, 调试用的话需要自己注入哦");
}
...
export let speakXunfei = (text, cb) => {
  ttsRecorder.setParams({ text, bgs: 0, speed: 86, voiceName: names[1] });
  ttsRecorder.start();
  ttsRecorder.callback = cb;
}
